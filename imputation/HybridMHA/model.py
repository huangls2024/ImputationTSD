"""
The implementation of HybridMHA for the partially-observed time-series imputation task.
...
class HybridMHA(BaseNNImputer):
    """The PyTorch implementation of the HybridMHA model.
    ...
    n_layers :
        The number of layers in the 1st and 2nd HybridMHA blocks in the SAITS model.

    d_model :
        The dimension of the model's backbone.
        It is the input dimension of the multi-head HybridMHA layers.

    n_heads :
        The number of heads in the multi-head HybridMHA mechanism.
        ``d_model`` must be divisible by ``n_heads``, and the result should be equal to ``d_k``.

    d_k :
        The dimension of the `keys` (K) and the `queries` (Q) in the HybridMHA mechanism.
    ...
    d_v :
        The dimension of the `values` (V) in the HybridMHA mechanism.
    ...
    attn_dropout :
        The dropout rate for HybridMHA.

    hybrid_attention_mask :
        Whether to apply a diagonal attention mask to the self-attention mechanism.
        If so, the attention layers will use HybridMHA. Otherwise, the attention layers will use the original.
    ...
    def predict(
        ...
        return_latent_vars :
            Whether to return the latent variables in SAITS, e.g. attention weights of two HybridMHA blocks and
        ...
        first_HybridMHA_attn_weights_collector = []
        second_HybridMHA_attn_weights_collector = []
        ...
        with torch.no_grad():
            for idx, data in enumerate(test_loader):
                ...
                results = self.model.forward(
                    inputs, hybrid_attention_mask, training=False
                )
                imputation_collector.append(results["imputed_data"])

                if return_latent_vars:
                    first_HybridMHA_attn_weights_collector.append(
                        results["first_HybridMHA_attn_weights"].cpu().numpy()
                    )
                    second_HybridMHA_attn_weights_collector.append(
                        results["second_HybridMHA_attn_weights"].cpu().numpy()
                    )
        ...
        if return_latent_vars:
            latent_var_collector = {
                "first_HybridMHA_attn_weights": np.concatenate(
                    first_HybridMHA_attn_weights_collector
                ),
                "second_HybridMHA_attn_weights": np.concatenate(
                    second_HybridMHA_attn_weights_collector
                ),

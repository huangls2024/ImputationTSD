...
from imputation import HybridMHA
from utils.metrics import calc_mae
from utils.metrics import calc_mse
from utils.metrics import calc_mre
from utils.metrics import calc_rmse
hybridmha = HybridMHA(n_steps=48, n_features=11, n_layers=2, d_model=256, n_heads=4, d_k=64, d_v=64, d_ffn=128, dropout=0.05, epochs=300)
# Here I use the whole dataset as the training set because ground truth is not visible to the model, you can also split it into train/val/test sets
hybridmha.fit(dataset)  # train the model on the dataset
imputation = hybridmha.impute(dataset)  # impute the originally-missing values and artificially-missing values

...
class HybridAttention(AttentionOperator):
    """Hybrid attention combining scaled dot-product and dynamic sparse attention.
        ...
        top_k_ratio:
            The ratio of top-k values to keep in the attention map for the sparse component.

        """

    def __init__(self, temperature: float, attn_dropout: float = 0.1, top_k_ratio: float = 0.1):
        super().__init__()
        assert temperature > 0, "temperature should be positive"
        assert attn_dropout >= 0, "dropout rate should be non-negative"
        assert 0 < top_k_ratio <= 1, "top_k_ratio should be in (0, 1]"
        self.temperature = temperature
        self.dropout = nn.Dropout(attn_dropout) if attn_dropout > 0 else None
    ...
    def forward(
        ...
        # Dynamic sparse attention component
        top_k = int(self.top_k_ratio * attn.size(-1))
        top_k_attn, _ = torch.topk(attn, top_k, dim=-1)
        min_top_k_attn = top_k_attn[:, :, :, -1].unsqueeze(-1)
        sparse_attn = torch.where(attn < min_top_k_attn, torch.full_like(attn, float('-inf')), attn)

        # Combine both attention maps
        attn = (attn + sparse_attn) / 2.0
    ...
    #
    #     top_k_ratio:
    #         The ratio of top-k values to keep in the attention map.
    #     """
    #
    # def __init__(self, temperature: float, attn_dropout: float = 0.1, top_k_ratio: float = 0.1):
    #     super().__init__()
    ...
    #         **kwargs,
    # ) -> Tuple[torch.Tensor, torch.Tensor]:
    #     """Forward processing of the dynamic sparse attention.
    #
    #     Parameters
    #     ----------
    #     q:
    ...
    #     -------
    #     output:
    #         The result of Value multiplied with the dynamic sparse attention map.
    #
    #     attn:
    #         The dynamic sparse attention map.
    #
    #     """
    #     q, k, v = q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2)
    #     attn = torch.matmul(q / self.temperature, k.transpose(2, 3))
    ...
    #     top_k_attn, _ = torch.topk(attn, top_k, dim=-1)
    #     min_top_k_attn = top_k_attn[:, :, :, -1].unsqueeze(-1)
    #     attn = torch.where(attn < min_top_k_attn, torch.full_like(attn, float('-inf')), attn)
    #
    #     attn = F.softmax(attn, dim=-1)
    #     if self.dropout is not None:
    #         attn = self.dropout(attn)
